...
public class HazardInvestigationModel : PageModel
{
    ...
    private async Task LoadInvestigatorsAsync()
    {
        try
        {
            ...
            AvailableInvestigators = investigators.Select(user => new SMSApplicationUser
            {
                Id = user.Id,
        ...
        catch (Exception ex)
        {
            ...
            AvailableInvestigators = new List<SMSApplicationUser>();
        }
    }
    ...
    private async Task<List<SMSApplicationUser>> LoadAvailableInvestigatorsAsync()
    {
        try
        {
            var smsUsers = _userRepository.GetAllSMSUsersAsync().Result;
            return smsUsers.Where(u => u.IsEmployee).Select(u => new SMSApplicationUser
            {
                Id = u.Id,
...
//public class SMSApplicationUser
//{
//    public string Id { get; set; } = string.Empty;
