        ...
        try
        {
            ...
            if (result.IsSuccess)
            {
                ...
                if (updatedMitigation is not null)
                {
                    ...
                    if (targetDateNotificationResult.IsFailure)
                    {
                        _logger.LogApplicationWarning(
                            "Failed processing mitigation target-date notifications for {MitigationCode}: {Error}",
                            updatedMitigation.Code,
                            targetDateNotificationResult.Error?.Message ?? "Unknown error");
                    }
                }
            }
            else
