...
public sealed class RiskAssessmentService : IRiskAssessmentService
{
    ...
    public async Task<Result<RiskAssessment>> CreateRiskAssessmentAsync(RiskAssessment assessment, CancellationToken ct = default)
    {
        try
        {
            _logger.LogApplicationInformation("Creating risk assessment with code: {Code}", assessment?.Code);
            var result = await _dataService.CreateRiskAssessmentAsync(assessment, ct).ConfigureAwait(false);

            if (result.IsSuccess)
            {
                _logger.LogApplicationInformation("Successfully created risk assessment with ID: {Id}", result.Value?.Id);
            }
            else
            {
                _logger.LogApplicationError("Failed to create risk assessment. Error: {Error}", result.Error?.Message);
            }
        ...
        {
            _logger.LogApplicationError(ex, "Unexpected error creating risk assessment");
    ...
    public async Task<Result<RiskAssessment>> GetRiskAssessmentByIdAsync(RiskAssessmentID id, CancellationToken ct = default)
    {
        try
        {
            _logger.LogApplicationInformation("Retrieving risk assessment with ID: {Id}", id);
            return await _dataService.GetRiskAssessmentByCodeAsync(id, ct).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            _logger.LogApplicationError(ex, "Unexpected error retrieving risk assessment with ID: {Id}", id);
    ...
    public async Task<Result<List<RiskAssessment>>> GetRiskAssessmentsByHazardCodeAsync(string hazardCode, CancellationToken ct = default)
    {
        try
        {
            _logger.LogApplicationInformation("Retrieving risk assessments for hazard: {HazardCode}", hazardCode);
        ...
        {
            _logger.LogApplicationError(ex, "Unexpected error retrieving risk assessments for hazard: {HazardCode}", hazardCode);
    ...
    public async Task<Result<RiskAssessment>> UpdateRiskAssessmentAsync(RiskAssessment assessment, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                _logger.LogApplicationError("Failed to update risk assessment. Error: {Error}", result.Error?.Message);
            }
        ...
        {
            _logger.LogApplicationError(ex, "Unexpected error updating risk assessment with ID: {Id}", assessment?.Id);
    ...
    public async Task<Result<bool>> DeleteRiskAssessmentAsync(RiskAssessmentID id, CancellationToken ct = default)
    {
        try
        {
            _logger.LogApplicationInformation("Deleting risk assessment with ID: {Id}", id);
            var result = await _dataService.DeleteRiskAssessmentAsync(id, ct).ConfigureAwait(false);

            if (result.IsSuccess)
            {
                _logger.LogApplicationInformation("Successfully deleted risk assessment with ID: {Id}", id);
            }
            else
            {
                _logger.LogApplicationError("Failed to delete risk assessment. Error: {Error}", result.Error?.Message);
            }
        ...
        {
            _logger.LogApplicationError(ex, "Unexpected error deleting risk assessment with ID: {Id}", id);
    ...
    public async Task<Result<string>> CalculateRiskLevelAsync(RiskAssessment assessment, CancellationToken ct = default)
    {
        try
        {
            _logger.LogApplicationInformation("Calculating risk level for assessment: {Id}", assessment?.Id);
            ...
            _logger.LogApplicationInformation("Calculated risk level {RiskLevel} for assessment {Id}", riskLevel, assessment.Id);
            return Result<string>.Success(riskLevel);
        }
        catch (Exception ex)
        {
            _logger.LogApplicationError(ex, "Unexpected error calculating risk level for assessment: {Id}", assessment?.Id);
    ...
    public async Task<Result<List<RiskAssessment>>> GetAllRiskAssessmentsAsync(CancellationToken ct = default)
    {
        try
        {
            _logger.LogApplicationInformation("Retrieving all risk assessments");
            return await _dataService.GetAllRiskAssessmentsAsync(ct).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            _logger.LogApplicationError(ex, "Unexpected error retrieving all risk assessments");
