...
//     Description: Command handlers for authentication audit operations.
//                  Processes authentication audit commands and coordinates with SystemService.
// </copyright>
//-----------------------------------------------------------------------
...
/// <summary>
/// Handler for recording successful authentication events
/// </summary>
public class RecordAuthenticationSuccessCommandHandler : BaseCommandBundle, IRequestHandler<RecordAuthenticationSuccessCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> HandleAsync(RecordAuthenticationSuccessCommand request, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("? Clean Architecture: Recording authentication success for user {UserName} ({UserType})", 
            ...
            var auditEntry = new AuditLogEntry(new AuditLogEntryID(Guid.NewGuid().ToString()))
            {
                ...
                Function = "UserLogin",
                Description = $"User '{request.UserDisplayName}' ({request.UserType}) successfully authenticated from {request.IPAddress}. Session: {request.SessionId}"
            };
            ...
            if (result.IsSuccess)
            {
                _logger.LogInformation("? Clean Architecture: Authentication success audit recorded for {UserName}", request.UserName);
...
/// <summary>
/// Handler for recording failed authentication attempts
/// </summary>
public class RecordAuthenticationFailureCommandHandler : BaseCommandBundle, IRequestHandler<RecordAuthenticationFailureCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> HandleAsync(RecordAuthenticationFailureCommand request, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogWarning("?? Security Alert: Authentication failure for user {UserName} from {IPAddress}", 
            ...
            var auditEntry = new AuditLogEntry(new AuditLogEntryID(Guid.NewGuid().ToString()))
            {
                ...
                Module = "SMS_Authentication", 
                Function = "UserLogin",
                Description = $"SECURITY ALERT: Failed authentication attempt for '{request.UserName}' from {request.IPAddress}. Reason: {request.FailureReason}. Attempt #{request.AttemptCount}"
            };
            ...
            if (result.IsSuccess)
            {
                _logger.LogInformation("? Clean Architecture: Authentication failure audit recorded for {UserName}", request.UserName);
                return Result<bool>.Success(true);
            }
            else
            {
...
/// <summary>
/// Handler for recording user logout events
/// </summary>
public class RecordLogoutCommandHandler : BaseCommandBundle, IRequestHandler<RecordLogoutCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> HandleAsync(RecordLogoutCommand request, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("? Clean Architecture: Recording logout for user {UserName} ({LogoutType})", 
            ...
            var auditEntry = new AuditLogEntry(new AuditLogEntryID(Guid.NewGuid().ToString()))
            {
                ...
                Module = "SMS_Authentication",
                Function = "UserLogout", 
                Description = $"User '{request.UserName}' ({request.UserType}) logged out ({request.LogoutType}). Session duration: {request.SessionDuration:hh\\:mm\\:ss}. Session: {request.SessionId}"
            };
            ...
            if (result.IsSuccess)
            {
                _logger.LogInformation("? Clean Architecture: Logout audit recorded for {UserName}", request.UserName);
                return Result<bool>.Success(true);
            }
            else
            {
