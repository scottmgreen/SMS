...
/// </summary>
public class HazardCreatedNotificationHandler : BaseUIEventHandler<TestUIEvent>
{
    ...
    /// </summary>
    protected override async Task<Result> ProcessUIEventAsync(TestUIEvent uiEvent, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("?? [UI HANDLER] Processing notification: {EventType} - {Message}", 
                uiEvent.EventType, uiEvent.Message);
            ...
            if (uiEvent.TestData != null)
            {
                _logger.LogInformation("?? [UI HANDLER] Notification Data: {Data}", 
                    System.Text.Json.JsonSerializer.Serialize(uiEvent.TestData, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
            }
