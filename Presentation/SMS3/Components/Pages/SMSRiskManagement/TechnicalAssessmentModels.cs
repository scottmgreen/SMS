...
public class Step3Model
{
    ...
    public async Task ApplyToAssessmentAsync(RiskAssessment assessment, IMediator mediator, List<Hazard> availableHazards)
    {
        ...
        try
        {
            ...
            // Save or Update RiskAnalysis entity for this assessment
            await SaveOrUpdateRiskAnalysisAsync(assessment, mediator);

            // Update individual hazards with analysis data
            await UpdateHazardsWithAnalysisDataAsync(availableHazards, mediator);
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Save or update the RiskAnalysis entity for this assessment
    /// </summary>
    private async Task SaveOrUpdateRiskAnalysisAsync(RiskAssessment assessment, IMediator mediator)
    {
        try
        {
            // TODO: Implement when RiskAnalysis CQRS commands are available
            // For now, we'll store the risk analysis data in the RiskAssessment entity
            
            Console.WriteLine($"Risk analysis method and criteria will be saved in RiskAssessment entity for {assessment.Code}");
            
            // The RiskAnalysisMethod and RiskCriteria are already applied to the assessment in the calling method
            
            /*
            // First, try to get existing RiskAnalysis for this assessment
            var getRiskAnalysisQuery = new GetRiskAnalysisQueryByAssessmentId(assessment.Id.Value);
            var existingResult = await mediator.SendAsync(getRiskAnalysisQuery, CancellationToken.None);

            if (existingResult.IsSuccess && existingResult.Value != null)
            {
                // Update existing RiskAnalysis
                var existingAnalysis = existingResult.Value;
                existingAnalysis.RiskAnalysisMethod = RiskAnalysisMethod;
                existingAnalysis.RiskCriteria = RiskCriteria;
            ...
            */
            ...
            Console.WriteLine($"RiskAnalysis data conceptually saved for assessment {assessment.Code}");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Error in RiskAnalysis placeholder: {ex.Message}");
            // Don't throw - continue with hazard updates
        }
    }
    ...
    /// </summary>
    private async Task UpdateHazardsWithAnalysisDataAsync(List<Hazard> availableHazards, IMediator mediator)
    {
        foreach (var hazard in availableHazards)
        {
            try
            {
                ...
                {
                    // Update hazard with analysis data
                    hazard.WorstCredibleOutcome = analysis.WorstCredibleOutcome;
                    hazard.RootCause = analysis.RootCauseAnalysis;
                    hazard.AdditionalComments = analysis.AdditionalComments;
                    hazard.UpdatedBy = "SYSTEM";
                    hazard.UpdatedDate = DateTime.UtcNow;

                    // Save hazard updates via CQRS
