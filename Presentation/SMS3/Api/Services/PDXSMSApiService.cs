...
namespace SMS3.Api.Services
{
    ...
    public class PDXSMSApiService : IPDXSMSApiService
    {
        ...
        public async Task<Result<PDXSMSReportApiResponse>> ProcessReportSubmissionAsyncV2(PDXSMSReportApiRequestV2 request,HttpContext httpContext)
        {
            try
            {
            ...
            HttpContext httpContext)
        {
            ...
            await QueueHazardStatusEscalationNotificationAsync(
                ...
                geoLocation,
                normalizedLocationDescription,
                validRecipientGroups);
        }

        private async Task QueueHazardStatusEscalationNotificationAsync(
            Hazard createdHazard,
            string baseUri,
            string? hazardDescription,
            string geoLocation,
            ...
            List<string> validRecipientGroups)
        {
            ...
            var subject = $"SMS Hazard Report Inaction Check - {createdHazard.ReportCode} / {createdHazard.Code}";
            var body = BuildHazardStatusEscalationEmailHtml(
                baseUri,
                createdHazard,
                hazardDescription,
                geoLocation,
            ...
            var escalationEvent = new EmailNotificationEvent(
                toRecipients: validRecipientGroups,
                subject: subject,
                body: body,
                isHtmlContent: true,
            ...
            var publishResult = await _eventBus.PublishIntegrationEventAsync(escalationEvent, executionMode);
            if (publishResult.IsFailure)
            {
                _logger.LogWarning(
                    "Failed to queue hazard status escalation notification for report {ReportCode}: {Error}",
                    createdHazard.ReportCode,
                    publishResult.Error?.Message ?? "Unknown publish error");
            }
        }
        ...
        private static string BuildHazardStatusEscalationEmailHtml(
            string baseUri,
            Hazard createdHazard,
            string? hazardDescription,
            string geoLocation,
            ...
            int thresholdHours)
        {
            ...
            return SMSEmailTemplateBuilder.BuildStandardEmail(
                ...
                introHtml: $"This is a follow-up status check generated at submission time. Send only if the report has not reached <strong>{ReportStatus.ReadyForProcessing.Value}</strong> by the SLA checkpoint.",
                summaryFields:
                [
                    new SMSEmailField { Label = "Report ID", Value = createdHazard.ReportCode ?? string.Empty },
                    new SMSEmailField { Label = "Hazard ID", Value = createdHazard.Code },
                    new SMSEmailField { Label = "Current Required Status", Value = ReportStatus.ReadyForProcessing.Value },
                    new SMSEmailField { Label = "Report Status Checkpoint (UTC)", Value = dueUtc.ToString("yyyy-MM-dd HH:mm:ss") }
                ],
                sections:
                [
                    new SMSEmailSection
                ...
                footerHtml: $"Status escalation threshold: {thresholdHours} hour(s).",
                logoUrl: logoUrl);
        }
