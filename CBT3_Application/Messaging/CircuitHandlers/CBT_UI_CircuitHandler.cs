...
public class CBT_UI_CircuitHandler : BaseCircuitHandler
{
    private ILogger<CBT_UI_CircuitHandler> _logger;
    public CBT_UI_CircuitHandler(ILogger<CBT_UI_CircuitHandler> logger, ILogSupport logsupport, IMediator mediator ) : base(logger, logsupport, mediator)
    {
        _logger = logger;
    }
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(circuit.Id))
            {
                ...
                if (circuit.Id != null)
                {
                    ...
                    trainingstation.Status = CBT3_Domain.Enums.TrainingStationStatus.MachineStopped;
