...
// Mini Map for Location Cards
window.initializeMiniMap = function (containerId, lat, lng) {
    try {
        const container = document.getElementById(containerId);
        if (!container) {
            console.error('Mini map container not found:', containerId);
            return;
        }
        container.innerHTML = '';

        // Create a small, non-interactive map
        const map = new ol.Map({
            target: containerId,
            layers: [
                new ol.layer.Tile({
            ...
            interactions: [], // Disable all interactions (non-interactive)
            controls: [] // Remove all controls for clean look
        });
        ...
        const markerLayer = new ol.layer.Vector({
            ...
            style: new ol.style.Style({
                image: new ol.style.Icon({
                    anchor: [0.5, 1],
                    scale: 0.8, // Smaller marker for mini map
                    src: 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(`
        ...
        // Store reference
        window[`miniMap_${containerId}`] = map;
        window[`miniMarkerSource_${containerId}`] = markerSource;
...
window.updateMiniMap = function (containerId, lat, lng) {
    try {
        const map = window[`miniMap_${containerId}`];
        const markerSource = window[`miniMarkerSource_${containerId}`];
        
        if (!map || !markerSource) {
            // Map not initialized yet, try to initialize
            window.initializeMiniMap(containerId, lat, lng);
            return;
        }
