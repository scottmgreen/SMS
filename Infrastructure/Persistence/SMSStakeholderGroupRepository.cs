...
public sealed class SMSStakeholderGroupRepository : BaseRepository<SMSStakeholderGroupRepository, SMSStakeholderGroup>
{
    ...
    public async Task<Result<IEnumerable<string>>> GetAllowedCompaniesByGroupCodeAsync(string groupCode, CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogInfrastructureGetItems($"{_logheader} {StoredProcs.pr_SMSStakeholderGroupCompany_GetByGroupCode} GroupCode:{groupCode}", null);

            using SqlConnection sql = new(_connectionString);
            using SqlCommand cmd = new(StoredProcs.pr_SMSStakeholderGroupCompany_GetByGroupCode, sql)
            {
    ...
    public async Task<Result<bool>> ReplaceAllowedCompaniesByGroupCodeAsync(string groupCode, IEnumerable<string> companyCodes, string updatedBy, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                using SqlCommand removeCmd = new(StoredProcs.pr_SMSStakeholderGroupCompany_Remove, sql)
                {
                    CommandType = CommandType.StoredProcedure
                };
            ...
            {
                using SqlCommand assignCmd = new(StoredProcs.pr_SMSStakeholderGroupCompany_Assign, sql)
                {
                    CommandType = CommandType.StoredProcedure
                };
